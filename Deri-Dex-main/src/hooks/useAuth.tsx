...
export function UserAuthProvider({
  ...
  const login = async () => {
    ...
    let deri =
      Math.floor((parseInt(tokenBal) / Math.pow(10, 18)) * 10000) / 10000;
  ...
  const uauth = new UAuth({
    ...
    scope:
      "openid wallet email social:optional social:telegram:optional social:twitter:optional",
  });
  ...
  return (
    <UserAuthContext.Provider
      value={{ account, login, logout, logon, loggout, isLoggedin }}
      children={children}
    />
  );
}
